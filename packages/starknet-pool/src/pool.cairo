...
///
/// Port of `L2PrivacyPool.sol`. Receives a note message and bridged tokens from the L1 pool over the
/// canonical Starknet messaging + StarkGate path (two independent, unordered ops), and lets a
/// recipient privately spend a delivered note to a clear address.
///
/// Safety rests on the same two guards as the EVM pool:
///   - Cross-domain auth: the `receive_note` l1_handler is invoked by the Starknet sequencer with the
///     L1 sender as `from_address`; we require it to equal the configured L1 pool.
///   - Backing invariant: a note is inserted as *pending* on arrival and becomes *spendable* only once
...
///   - Single ERC20 path (StarkGate delivers an L2 ERC20; even ETH is an ERC20) — no native branch.
///   - `context`/`scope` are BN254-Poseidon folds, not keccak(abi.encode(...)) — see `hashing.cairo`.
...
pub mod StarknetPrivacyPool {
    ...
    use crate::interfaces::{
        IStarknetPrivacyPool, Withdrawal, IERC20Dispatcher, IERC20DispatcherTrait,
    };
    ...
    fn constructor(
        ...
        self
            ...
            .write(
                poseidon_fold(
                    array![
                        addr_to_u256(get_contract_address()), chain_id, addr_to_u256(asset),
                    ]
                        .span(),
                ),
            );
    ...
    /// `from_address` is the L1 sender. Payload on the wire is `[value, commitment_lo, commitment_hi]`
    ...
    fn receive_note(
        ...
        self.pending_value.write(commitment, value_u256);

        self.emit(NoteReceived { commitment, value: value_u256 });
    ...
    impl StarknetPrivacyPoolImpl of IStarknetPrivacyPool<ContractState> {
        ...
        fn withdraw(
            ...
            let pubs: Span<u256> = match verifier
            ...
            assert(withdrawal.relay_fee_bps <= self.max_relay_fee_bps.read(), 'RelayFeeGreaterThanMax');
    ...
    impl InternalImpl of InternalTrait {
        /// Activate a pending note if its bridged tokens have landed. Enforces the backing invariant
        /// `activated_supply + value <= tokens_received_from_bridge`.
        ...
        /// The total tokens received from the bridge (still held plus already withdrawn); the ceiling
        /// for spendable supply. Nothing but the bridge adds asset to this pool.
        ...
        /// Insert a leaf into the LeanIMT and record the new root. Mirrors `InternalLeanIMT._insert`.
        ...
        fn _compute_context(self: @ContractState, w: Withdrawal) -> u256 {
            poseidon_fold(
                array![
                    addr_to_u256(w.processooor),
                    addr_to_u256(w.recipient),
                    addr_to_u256(w.fee_recipient),
                    w.relay_fee_bps,
                    self.scope.read(),
                ]
                    .span(),
